class Note {
  ...
  class App {
    ...
    closeModal(event) {
        const isModalFormFormClickedOn = this.$modalForm.contains(event.target);
        const isCloseModalBtnClickedOn = this.$closeModalForm.contains(event.target);
        console.log(isCloseModalBtnClickedOn); 
        if((!isModalFormFormClickedOn || isCloseModalBtnClickedOn) && this.$modal.classList.contains("open-modal")) {
